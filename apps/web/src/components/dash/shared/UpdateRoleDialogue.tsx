...
export default function UpdateRoleDialogue({userID, currentRole, setOpen}:{userID:number, currentRole:MemberType, setOpen:React.Dispatch<React.SetStateAction<boolean>>}) {
  const [role,setRole] = useState(currentRole);
  const {execute:runUpdateMemberRole, status} = useAction(updateMemberRole,{
    onSuccess:()=>{
      setOpen(false);
      toast.dismiss();
    ...
    onError:(err)=>{
      ...
      return toast.error("You need super admin permissions to update roles")
    }
  ...
  return (
		<DialogContent>
			...
			<Select onValueChange={(v)=>setRole(v as MemberType)} defaultValue={role}>
