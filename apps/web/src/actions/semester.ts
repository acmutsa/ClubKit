...
import { createSemesterSchema, toggleCurrentSemesterSchema, updateSemesterSchema } from "db/zod"
import { UNIQUE_KEY_CONSTRAINT_VIOLATION_CODE } from "@/lib/constants"
import { SEMESTER_DATE_RANGE_EXISTS, SEMESTER_NAME_EXISTS } from "@/lib/constants/semesters"
import { resetCurrentSemesters, getExistingSemester } from "@/lib/queries/semesters"
import z from "zod"
...
export const createNewSemester = executiveAction
  ...
  .action(async ({parsedInput}) => {
    ...
    try{
      const res = await db.insert(semesters).values(parsedInput).returning({semesterID:semesters.semesterID});
